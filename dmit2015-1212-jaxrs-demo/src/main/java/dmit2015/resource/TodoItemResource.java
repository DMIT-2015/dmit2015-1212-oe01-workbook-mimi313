...
public class TodoItemResource {
    ...
    public Response postTodoItem(TodoItem newTodoItem) { // Need a name as it has NotBlank annotation elsewhere
        if (newTodoItem == null) {
    ...
    @Path("{id}")
    public Response getTodoItem(@PathParam("id") Long id) { //@Path and @PathParam have to match
...
//        }
//        TodoItem existingTodoItem = optionalTodoItem.get();
        // the block of the code is equivalent to this line of code below
        TodoItem existingTodoItem = todoItemRepository.findOptional(id).orElseThrow(NotFoundException::new);

        // Copy data from the updated entity to the existing data
        ...
        todoItemRepository.update(existingTodoItem);
            ...
            return Response
                    .status(Response.Status.BAD_REQUEST)
                    .entity("You are updating an old version of the data. Please fetch the new version.")
                    .build();
        } catch (Exception ex) {
            return Response
                    .serverError()
